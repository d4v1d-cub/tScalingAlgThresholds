...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    fprintf(stderr, "usage: %s N alpha maxIter numSamples\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  N = atoi(argv[1]);
  alpha = atof(argv[2]);
  ...
  printf("# T=0 quenches for random K-SAT\n");
  printf("# K = %i   N = %i   M = %i   seed = %u\n",
	 K, N, M, myrand);
  printf("# alpha = %f   maxIter = %i\n", (float)M/N, maxIter);
  printf("# 1:timeToSolution\n");
  initRandom();
  for (is = 0; is < numSamples; is++) {
    while(initGraph());
    initSpin();
    i = 0;
    while (i < maxIter && numUnsat) {
      oneMCS_T0();
      i++;
    }
    printf("%i\n", i);
