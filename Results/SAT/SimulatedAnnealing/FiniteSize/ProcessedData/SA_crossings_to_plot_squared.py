...
def get_fits(path_to_data, filefits, thr, npars, npoints):
    ...
    gamma_cross, chi_sqr_cross, pars_cross = get_info_pos(data, pos, npars, deg_freedom)

    print("chi_sqr:", chi_sqr_left, chi_sqr_min, chi_sqr_right, chi_sqr_cross)
    print("gamma:", gamma_left, gamma_min, gamma_right, gamma_cross)
    return gamma_min, [gamma_left, gamma_right], pars_min, [pars_left, pars_right], gamma_cross, pars_cross
...
def export_data(min_n, params_min, params_list, gamma_min, gamma_list, 
                gamma_cross, pars_cross, path_to_data, fileout, ncurves, nsamples):
    ...
    fraction =  gamma_cross / gamma_min
    xvals_trans = xvals ** fraction
    for i in range(ncurves):
        for j in range(nsamples):
            yvals[-1, i, j] = func_to_fit(xvals_trans[j], pars_cross[i][0], pars_cross[i][2])        

    for j in range(nsamples):
        fout.write(str("{0:.6f}".format(xvals[j])))
        for g in range(len(gamma_list) + 2):
            for i in range(ncurves):
...
def main():
    ...
    fileout = f'data_to_plot_fits_squared_simultaneous.txt'
    npoints = 9
    npars = 4
    ncurves = 2

    gamma_min, gamma_list, params_min, params_list, gamma_cross, pars_cross = \
